...
vec3 prefilterEnvMap(vec3 R, float roughness)
{
	...
	vec3 color = vec3(0.0);
	float totalWeight = 0.0;
	float envMapDim = float(textureSize(environment_map, 0).s);
	...
	for(uint i = 0u; i < SAMPLE_COUNT; i++) {
		...
		if(dotNL > 0.0) {
			...
			color += textureLod(environment_map, L, mipLevel).rgb * dotNL;
			totalWeight += dotNL;
...
{		
    ...
    for(uint i = 0u; i < SAMPLE_COUNT; ++i)
    {
        vec2 Xi = vec2(float(i) / float(SAMPLE_COUNT), fract(float(i) * 0.618033988749895f));
        vec3 H = ImportanceSampleGGX(Xi, N, roughness);
        vec3 L = normalize(2.0 * dot(V, H) * H - V);
